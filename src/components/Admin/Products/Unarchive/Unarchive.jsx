...
const Unarchive = () => {
	...
	const proceedHandler = async (e) => {
		...
		setIsBtnActive(false);
		await fetch(
			`https://labloco-medical-supplies.herokuapp.com/products/unarchive/${fetchedProductId}`,
			{
				method: "PUT",
				headers: {
					"Content-Type": "application/json",
				...
				<Col className="p-0 m-0">
					...
					<Modal show={show} onHide={handleClose}>
						<Modal.Header closeButton>
							<Modal.Title>Unarchive a product</Modal.Title>
						</Modal.Header>
						<Modal.Body>
							<Form className="gap-3 d-flex flex-column">
								<Form.Group>
									<Form.Label>Select product a archive</Form.Label>
									<Form.Select
										value={product}
